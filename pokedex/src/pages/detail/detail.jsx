...
export const Detail = ({list}) => {
    ...
    const getSurroundingPokemon = () => {
        if (pokemon.number !== 1) setPreviousPokemonNumber(pokemon.number - 1);
        if (pokemon.number !== Math.max(...list.map((pokemon) => pokemon.number))) setNextPokemonNumber(pokemon.number + 1);
    }
